...
public class FuncionarioDTO {
    ...
    @Pattern(regexp = "^\\d{11}$", message = "A CNH deve ter 11 dígitos")
    private String numeroCnh;

    @Future(message = "A validade da CNH deve ser futura")
    ...
    @Pattern(regexp = "^\\d{7}-\\d{2}\\.\\d{4}-\\d{2}$", message = "Formato de CTPS inválido (XXXXXXX-XX.XXXX-XX)")
    private String ctps;

    @Pattern(regexp = "^\\d{4} \\d{4} \\d{4}$", message = "Formato de título de eleitor inválido (XXXX XXXX XXXX)")
